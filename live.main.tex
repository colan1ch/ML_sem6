...
% Настройка стилей для отображения кода Питера
\lstset{
    language=Python,
...
% --- ОТЧЕТ ---
\section{Цель работы}
1. Исследование необходимых и достаточных условий существования экстремума функции с учетом ограничений (условный экстремум) \cite{cite: 5}.\\
2. Вычисление экстремумов функции \cite{cite: 5}.

\section{Постановка задачи}
Исследовать на условный локальный экстремум целевую функцию:
$$f(x) = (10\ln x_1 - 2)^2 + 10(x_2^2 + 2)^2 - 10 \rightarrow \text{extr}$$
при ограничениях типа неравенств \cite{cite: 40}:
$$\begin{cases}
...
Составим классическую функцию Лагранжа ($\lambda_0=1$) \cite{cite: 70, cite: 94}:
$$L(x, \lambda) = (10\ln x_1 - 2)^2 + 10(x_2^2 + 2)^2 - 10 + \lambda_1(x_1^2 + x_2^2 - 2) + \lambda_2(-x_1) + \lambda_3(-x_2)$$

Запишем систему уравнений стационарности \cite{cite: 98}:
...
Условия дополняющей нежесткости \cite{cite: 99}:
$$\lambda_1(x_1^2 + x_2^2 - 2) = 0, \quad \lambda_2(-x_1) = 0, \quad \lambda_3(-x_2) = 0$$
при $\lambda_j \ge 0$ для минимума \cite{cite: 99}.
...
Проверка допустимости $g_1(x^e)$: $(e^{0.2})^2 + 0^2 - 2 = e^{0.4} - 2 \approx -0.508 \le 0$ (выполняется) \cite{cite: 58}.\\
Получена точка $x^e = (e^{0.2}, 0)$ при $\lambda^e = (0, 0, 0)^T$.

\subsection{Проверка достаточных условий второго порядка}
Вычислим матрицу вторых производных (Гессиан) функции Лагранжа в точке $x^e$ \cite{cite: 112}:
...
Матрица $H = \begin{pmatrix} \frac{200}{e^{0.4}} & 0 \\ 0 & 80 \end{pmatrix}$ положительно определена. Второй дифференциал $d^2L > 0$ для любых ненулевых приращений \cite{cite: 121}, следовательно, $x^e$ — точка условного локального \textbf{минимума} \cite{cite: 121}.

Значение функции в точке экстремума \cite{cite: 124}:
